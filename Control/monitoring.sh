...
#echo 'Установлена временная зона Москва - Ok'
...
#1. вносим изменения в файл конфигурации Прометея
...
#2. вносим изменения в файл конфигурации Графана
cpFileName='prometheus.yml' 
...
destinDir=/etc/prometheus
destinFile=$destinDir/$cpFileName
description="конфигурации сервиса Grafana"
if [ -f $destinFile ]
then 
  mv $destinFile $destinFile.bak
fi 
...
for service in  prometheus-node-exporter prometheus  grafana-server
do
